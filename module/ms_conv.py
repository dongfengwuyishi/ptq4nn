...
def _unpack(x):
    """兼容 QuantConv2d 的 (tensor, scale) 输出和普通 Conv2d 的 tensor 输出"""
    if isinstance(x, tuple):
        return x[0], x[1]
    return x, None


def _repack(x, s, target=None):
    """有 scale 且 target 是 QuantMultiStepLIFNode 时打包为元组，否则返回原 tensor"""
    if s is not None and target is not None and hasattr(target, 'mem_fake_quant'):
        return (x, s)
    return x
...
class MS_MLP_Conv(nn.Module):
    ...
    def forward(self, x, hook=None):
        ...
        # fc1_conv 输出的 scale 将传给 fc2_lif（数据流: fc1_conv → bn → fc2_lif）
        x, _s_fc1 = _unpack(self.fc1_conv(x.flatten(0, 1)))
        x = self.fc1_bn(x).reshape(T, B, self.c_hidden, H, W).contiguous()
        if self.res:
            x = identity + x
            identity = x

        x = self.fc2_lif(_repack(x, _s_fc1, self.fc2_lif))
        if hook is not None:
            hook[self._get_name() + str(self.layer) + "_fc2_lif"] = x.detach()
        x, _s_fc2 = _unpack(self.fc2_conv(x.flatten(0, 1)))
...
class MS_SSA_Conv(nn.Module):
    ...
    def forward(self, x, hook=None):
        ...
        q_conv_out, _sq = _unpack(self.q_conv(x_for_qkv))
        q_conv_out = self.q_bn(q_conv_out).reshape(T, B, C, H, W).contiguous()
        q_conv_out = self.q_lif(_repack(q_conv_out, _sq, self.q_lif))
        ...
        k_conv_out, _sk = _unpack(self.k_conv(x_for_qkv))
        k_conv_out = self.k_bn(k_conv_out).reshape(T, B, C, H, W).contiguous()
        k_conv_out = self.k_lif(_repack(k_conv_out, _sk, self.k_lif))
        if self.dvs:
        ...
        v_conv_out, _sv = _unpack(self.v_conv(x_for_qkv))
        v_conv_out = self.v_bn(v_conv_out).reshape(T, B, C, H, W).contiguous()
        v_conv_out = self.v_lif(_repack(v_conv_out, _sv, self.v_lif))
        if self.dvs:
        ...
        x, _ = _unpack(self.proj_conv(x.flatten(0, 1)))
        x = (
            self.proj_bn(x)
            .reshape(T, B, C, H, W)
            .contiguous()
