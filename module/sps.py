...
class MS_SPS(nn.Module):
    ...
    def forward(self, x, hook=None):
        ...
        x, _s = _unpack(self.proj_conv(x.flatten(0, 1)))
        x = self.proj_bn(x).reshape(T, B, -1, H // ratio, W // ratio).contiguous()
        x = self.proj_lif(_repack(x, _s, self.proj_lif))
        if hook is not None:
        ...
        x, _s = _unpack(self.proj_conv1(x))
        x = self.proj_bn1(x).reshape(T, B, -1, H // ratio, W // ratio).contiguous()
        x = self.proj_lif1(_repack(x, _s, self.proj_lif1))
        if hook is not None:
        ...
        x, _s = _unpack(self.proj_conv2(x))
        x = self.proj_bn2(x).reshape(T, B, -1, H // ratio, W // ratio).contiguous()
        x = self.proj_lif2(_repack(x, _s, self.proj_lif2))
        if hook is not None:
        ...
        x, _s3 = _unpack(self.proj_conv3(x))
        x = self.proj_bn3(x)
        ...
        x_feat = x
        x = self.proj_lif3(_repack(
            x.reshape(T, B, -1, H // ratio, W // ratio).contiguous(), _s3,
            self.proj_lif3
        ))
        if hook is not None:
        ...
        x, _s_rpe = _unpack(self.rpe_conv(x))
        x = self.rpe_bn(x)
