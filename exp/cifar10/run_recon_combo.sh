#!/usr/bin/env bash
# ==========================================================================
# Reconstruction combo sweep (CIFAR10)
# ==========================================================================
...
#
# Usage:
#   GPU=0 ./exp/cifar10/run_recon_combo.sh
# ==========================================================================
...
# Chosen from the strongest CIFAR10 one-factor results:
# reg1e-5, b16, mem0.10/mem0.50, lr1e-3.
EXPS=(
  "reg1e5_b16_mem010_lr1e3|recon_reg_lam_scale=0.00001|recon_num_batches=16|recon_mem_lam=0.10|adaround_lr=0.001"
  "reg1e5_b16_mem010_lr3e3|recon_reg_lam_scale=0.00001|recon_num_batches=16|recon_mem_lam=0.10|adaround_lr=0.003"
  "reg1e5_b16_mem050_lr1e3|recon_reg_lam_scale=0.00001|recon_num_batches=16|recon_mem_lam=0.50|adaround_lr=0.001"
  "reg1e5_b16_mem025_lr1e3|recon_reg_lam_scale=0.00001|recon_num_batches=16|recon_mem_lam=0.25|adaround_lr=0.001"
  "reg0_b16_mem010_lr1e3|recon_reg_lam_scale=0.0|recon_num_batches=16|recon_mem_lam=0.10|adaround_lr=0.001"
  "reg1e5_b16_mem000_lr1e3|recon_reg_lam_scale=0.00001|recon_num_batches=16|recon_mem_lam=0.0|adaround_lr=0.001"
)
...
for spec in "${EXPS[@]}"; do
  ...
  tmp="$(mktemp "${TMPDIR:-/tmp}/ptq_cifar10_combo_${tag}.XXXXXX.yml")"
  cp "${BASE_CONFIG}" "${tmp}"
  ...
  if [[ -z "${last_dir}" || ! -f "${last_dir}/ptq.log" ]]; then
    last_dir="$(ls -1dt "${OUT_ROOT}"/*-sdt-w*-cifar10 2>/dev/null | head -n1 || true)"
  fi
