...
# 用法（在仓库根或本目录执行均可）:
#   ./exp/cifar100/run_scale_bridge_sweep.sh
#   SCALE_BRIDGES="weight unify" ./exp/cifar100/run_scale_bridge_sweep.sh
...
CONFIG="${CONFIG:-exp/cifar100/config.yml}"
if [[ ! -f "${CONFIG}" ]]; then
...
for bridge in ${SCALE_BRIDGES}; do
  ...
  tmp="$(mktemp "${TMPDIR:-/tmp}/ptq_cifar100_${bridge}.XXXXXX.yml")"
