...
# 用法（在仓库根或本目录执行均可）:
#   ./exp/imageNet/run_scale_bridge_sweep.sh
#   SCALE_BRIDGES="weight unify" ./exp/imageNet/run_scale_bridge_sweep.sh
...
CONFIG="${CONFIG:-exp/imageNet/config.yml}"
if [[ ! -f "${CONFIG}" ]]; then
...
DEFAULT_BRIDGES="unify weight pot observer"
...
for bridge in ${SCALE_BRIDGES}; do
  ...
  tmp="$(mktemp "${TMPDIR:-/tmp}/ptq_imagenet_${bridge}.XXXXXX.yml")"
